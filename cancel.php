...
mysql_connect("localhost","root","");
	...
	$r="SELECT * from tickets where pnr='$x';";
	$res=mysql_query($r);
	$row=mysql_fetch_array($res);
	$tno=$row[2];
	$q2="UPDATE trains SET genseats = genseats+$z WHERE tno='$tno';";		
	mysql_query($q2);
	if($z==$y)
	{
	$q="DELETE from tickets where pnr='$x';";		
	mysql_query($q);
	echo "You successfully cancelled your previous booking of $y tickets.<br>See you soon.!!<br>";
	}
	else
	{
	$q="UPDATE tickets SET ticket = $y-$z WHERE pnr='$x';";		
	mysql_query($q);
	echo "You successfully cancelled $z tickets from your previous booking of $y tickets.<br>Happy Journey.!!<br>";
	}
}
?>
</td>
